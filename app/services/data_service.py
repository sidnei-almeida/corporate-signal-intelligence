...
def _load_csv(filename: str) -> pd.DataFrame:
    """Load and cache a CSV file from the data directory (immutable cache)."""
...
@lru_cache(maxsize=1)
def _cached_anomaly_results() -> pd.DataFrame:
    """Cached, normalized anomaly results (do not mutate — use load_anomaly_results)."""
...
@lru_cache(maxsize=1)
def _cached_company_universe() -> frozenset[str]:
    """Cached set of known tickers for O(1) existence checks."""
    df = _cached_anomaly_results()
    if df.empty or "ticker" not in df.columns:
        return frozenset()
    return frozenset(df["ticker"].dropna().astype(str).tolist())
...
def load_model_ready_dataset() -> pd.DataFrame:
    ...
    return _load_csv(MODEL_READY_FILE).copy()


def load_market_features() -> pd.DataFrame:
    """Load market feature dataset."""
    return _load_csv(MARKET_FEATURES_FILE).copy()


def load_filing_features() -> pd.DataFrame:
    """Load filing feature dataset."""
    return _load_csv(FILING_FEATURES_FILE).copy()


def load_financial_features() -> pd.DataFrame:
    """Load selected financial features."""
    return _load_csv(FINANCIAL_FEATURES_FILE).copy()


def _load_top_anomalies_file() -> pd.DataFrame:
    """Load top anomalies CSV as a defensive copy."""
    path = _csv_path(TOP_ANOMALIES_FILE)
    if not path.is_file():
        return pd.DataFrame()
    return _normalize_anomaly_frame(_load_csv(TOP_ANOMALIES_FILE)).copy()
...
def _filter_ticker(df: pd.DataFrame, ticker: str) -> pd.DataFrame:
    """Filter a DataFrame by normalized ticker (never mutates the input)."""
    if df.empty or "ticker" not in df.columns:
        return df.copy()
    normalized = normalize_ticker(ticker)
    return df.loc[df["ticker"] == normalized].copy()
...
def get_top_anomalies(limit: int = 20) -> pd.DataFrame:
    ...
    if "is_anomaly" in working.columns:
        working = working.loc[working["is_anomaly"] == True].copy()  # noqa: E712
    if "anomaly_score" in working.columns:
        working = working.sort_values("anomaly_score", ascending=True).copy()
    return working.head(limit).copy()
...
def find_anomaly_record(ticker: str, date_value: str) -> dict[str, Any] | None:
    ...
    matches = company_df.loc[company_df["date"].dt.date == target.date()].copy()
    if matches.empty:
        return None
...
def query_anomalies(
    ...
    if ticker:
        normalized = normalize_ticker(ticker)
        df = _filter_ticker(df, normalized)
    if df.empty:
        return []

    working = df.copy()
    if only_anomalies and "is_anomaly" in working.columns:
        working = working.loc[working["is_anomaly"] == True].copy()  # noqa: E712

    if sort_by in working.columns:
        working = working.sort_values(sort_by, ascending=ascending, na_position="last").copy()
    elif "anomaly_score" in working.columns:
        working = working.sort_values("anomaly_score", ascending=True, na_position="last").copy()

    return dataframe_to_records(working.head(limit))
